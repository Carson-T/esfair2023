...
def transform(args):
    train_transforms = albumentations.Compose([
        ...
        albumentations.CLAHE(clip_limit=0.5, p=0.2),
        albumentations.HueSaturationValue(hue_shift_limit=10, sat_shift_limit=20, val_shift_limit=10, p=0.5),
        albumentations.ShiftScaleRotate(shift_limit=0.1, scale_limit=0.1, rotate_limit=15, border_mode=0, p=0.5),
        albumentations.CoarseDropout(max_holes=1, max_height=int(args["resize"] * 0.1), max_width=int(args["resize"] * 0.1), min_height=1, min_width=1, p=0.2),
